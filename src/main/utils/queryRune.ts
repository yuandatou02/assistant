import type { Block, OnlineRunes } from "@/lcu/types/RuneTypes";
...
export class QueryRune {
    ...
    public getChampInfo = async (alias: string): Promise<OnlineRunes[]> => {
        ...
        if (this.mapId === 12) {
            console.log(`${baseUrl}/op.gg-aram/${alias}.json?date${timestamp}`);
            const res = await requestFetch<any>(
                `${baseUrl}/op.gg-aram/${alias}.json?date${timestamp}`,
                "GET",
            );
            return res === null ? [] : res;
        } else {
            console.log(`${baseUrl}/op.gg-aram/${alias}.json?date${timestamp}`);
            const res = await requestFetch<any>(
                `${baseUrl}/op.gg/${alias}.json?date${timestamp}`,
                "GET",
            );
            return res === null ? [] : res;
        }
    ...
    public handleBlocks = (blocks: Block[]) => {
        ...
        for (const block of blocks) {
            if (block.type.indexOf("Starter") !== -1) {
                ...
                blocksResult.push(
                    this.translateTitle(block, "Core Items,", "核心:"),
                );
            }
        }
        return blocksResult;
    ...
    public getRunesData = async (alias: string) => {
        try {
            this.mapId = JSON.parse(
                localStorage.getItem("gameInfo") as string,
            ).mapId;
            const champInfo = await this.getChampInfo(alias);
            // 技能
            ...
            for (const champ of champInfo) {
                ...
                // 出装
                const block = this.getBlocksData(
